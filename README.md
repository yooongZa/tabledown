...
LLM 프롬프트에 표를 넣을 때 잘 인식되는 **레코드형 + 구조 태그 XML**을 지원합니다. 루트는 `<dataset>`, 값마다 `<cell name="컬럼명">`, 다단(그룹) 헤더는 `<group>` 으로 중첩해 표의 계층을 그대로 보존합니다.
...
- **이름은 `name=` 속성, 값은 `<cell>` 텍스트**: 헤더에 공백·기호·숫자가 있어도 태그가 망가지지 않고 어떤 표준 XML 파서로도 안전합니다(현실의 표 표준과 같은 방식). 가로 다단 헤더는 `<group>` 으로 중첩하고, 세로 병합은 각 행에 값을 채워 보존합니다. 루트가 `<dataset>` 이라 브라우저·Obsidian 미리보기처럼 HTML 로 렌더링되는 곳에서도 내용이 사라지지 않습니다.
