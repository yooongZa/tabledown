...
‘선택한 표를 XML로 복사’를 명시적으로 실행하면 현재 Excel 선택 영역의 셀 값·빈칸·병합 구조를, ‘표의 수식을 포함해 XML로 복사’를 실행하면 셀 값·빈칸·수식·주소를 로컬에서 읽어 같은 clipboard에 XML text로 기록합니다. 셀 값과 수식 내용은 로그에 남지 않고 외부 서버로 전송되지 않습니다.
