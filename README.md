...
| Excel 표 범위 선택 후 메뉴 ‘표 구조·표시값을 XML로 복사’ 또는 `⌘⌃X` | LLM 프롬프트 (`Cmd+V`) | 계층·실제 병합 범위·출처·표시값 XML |
| Excel 수식 영역 선택 후 메뉴 ‘셀 값·수식·참조를 XML로 복사’ 또는 `⌘⌃E` | LLM 프롬프트 (`Cmd+V`) | 셀 타입·원시값·계산 상태·A1/R1C1 수식·직접 참조값 XML |
| Excel 수식 영역 선택 후 메뉴 ‘AI용 간결 복사’ | LLM 프롬프트 (`Cmd+V`) | 기존 수식 정보·추정 맥락·중복을 줄인 공통 참조 XML |
...
LLM 프롬프트에 표를 넣을 때 잘 인식되는 **다단 헤더 중첩 XML**을 지원합니다. 다단(그룹) 헤더를 XML 중첩 계층으로 표현합니다 — 루트는 `<표>`, 세로 그룹은 `<{헤더}그룹 이름="값">`, 행은 `<행 {헤더}="값">`, 가로 그룹은 `<열그룹 이름="값">`, 셀은 `<열 n="헤더값">셀값</열>`. 이 중첩은 Excel 병합과 값 배치를 바탕으로 한 structural inference(구조 추론)이며, 실제 병합 범위는 루트의 `병합범위`가 별도로 보존합니다.
...
- **진행 상태와 clipboard 덮어쓰기 방지**: XML 명령은 먼저 메뉴바 아이콘 옆에 `…`를 표시하고 메뉴를 ‘복사 중…’ 상태로 바꾼 뒤 한 번에 하나만 실행됩니다. macOS의 NSAppleScript가 main thread(메인 스레드) 전용이라 큰 범위, 특히 10,000셀에 가까운 선택은 수십 초 동안 메뉴가 응답하지 않을 수 있고 현재 작업 중간 취소는 지원하지 않습니다. 오래 기다리면 범위를 나눠 실행하세요. 준비 중 다른 앱에서 새 내용을 복사하면 마지막 generation check(세대 확인)에서 XML 쓰기를 취소하며, 실제 기록을 다시 확인한 뒤에만 성공 체크 표시를 보여줍니다. macOS clipboard API에는 atomic compare-and-swap(원자적 비교 후 쓰기)이 없어 마지막 확인과 비우기 사이의 아주 작은 경쟁 창까지 절대적으로 없앨 수는 없습니다.
...
- **‘빈칸을 자동 채우기’ (설정 ▸, 기본 꺼짐)**: 병합 없이 빈칸으로 그룹을 표현한 표를 변환할 때, 마크다운·XML **두 변환 경로 공통**으로 왼쪽 그룹(키) 열과 헤더 프레임의 빈칸을 바로 위(세로)→좌측(가로) 값으로 채웁니다. 데이터(값) 영역의 빈칸은 그대로 둡니다. 일반 XML 루트는 `빈칸채움="적용|미적용"`·`빈칸채움기준`·`빈칸채움수`·`빈칸채움셀`에 적용 여부와 실제로 값이 생성된 원본 A1 주소를 기록하므로, 보정값을 원본 입력값으로 오인하지 않아도 됩니다.

## 셀 값·수식·참조를 XML로 복사 (Excel 전용)

Excel에서 수식이 들어 있는 단일 사각형 영역을 선택한 뒤 메뉴바의 **‘셀 값·수식·참조를 XML로 복사’** 를 누르거나 전역 단축키 **⌘⌃E** 를 누르면, 선택 영역의 모든 셀을 행·열 구조 그대로 XML text(일반 텍스트)로 복사합니다. 상수 값·빈 셀·셀 주소와 수식 셀의 현재 결과·A1·R1C1 수식이 함께 들어갑니다. 수식에 직접 적힌 같은 통합문서의 static A1 reference(정적 A1 참조)는 같은 시트와 다른 시트 모두 현재 값까지 수식 셀 아래에 연결합니다. 병합 계층과 표시 서식은 포함하지 않으며, 먼저 `Cmd+C`를 누를 필요는 없습니다.
...
- `<참조범위>`는 해당 수식에 직접 적힌 같은 통합문서의 A1 셀·범위와 현재 값을 연결합니다. 같은 범위를 여러 수식이 공유하면 Excel에서는 한 번만 읽되, XML에서는 각 수식에 참조가 적힌 순서를 그대로 유지합니다. 다른 시트도 `시트` 속성으로 구분되며, 범위 참조의 내부 `<참조셀>`은 행 우선 순서입니다.
...
- `값기준="Excel현재원시값"`이며 수식 XML에는 표시 문자열·숫자 서식·병합 계층을 넣지 않습니다. 이 제한은 `표시정보상태="미포함"`·`병합정보상태="미포함"`으로 XML 자체에 표시됩니다. 화면에 보이는 날짜·통화·퍼센트와 실제 병합 구조가 필요하면 일반 **‘표 구조·표시값’** XML을 함께 사용하세요.
...
수식이 포함된 영역을 선택하고 메뉴의 **‘AI용 간결 복사’** 를 누릅니다. 제목과 항목명도 함께 선택하면 AI가 값의 의미를 파악하는 데 도움이 됩니다. 기존 수식 복사와 같은 stable snapshot(안정된 스냅샷) 읽기 경로를 사용하며, 값·수식·참조값·계산 상태·누락 사유와 기존 크기 한도를 보존합니다.
...
- 시트와 주소가 정확히 같은 참조범위만 `<참조목록>`에 한 번 기록하고 각 수식의 `<참조 ref="…">`로 연결합니다. 일부만 겹치는 범위는 각각 유지하며, 수식별 참조 순서도 보존합니다. 반복 참조가 많을수록 길이가 줄고 작은 표는 맥락 정보 때문에 더 길어질 수 있습니다.
- 기존 수식 XML 메뉴·`⌘⌃E`·자동 변환과 기본 설정은 유지됩니다. 새 설정이나 단축키는 없으며, 결과를 로컬 클립보드에 복사합니다. Tabledown이 AI에 전송하지 않습니다.
...
기본 표 변환에는 손쉬운 사용(Accessibility)이나 입력 모니터링(Input Monitoring) 권한이 필요하지 않습니다. ‘표 구조·표시값을 XML로 복사’·‘셀 값·수식·참조를 XML로 복사’·‘AI용 간결 복사’는 현재 Excel 선택 영역을 직접 읽으므로 처음 사용할 때 Microsoft Excel Automation(자동화) 권한을 요청할 수 있습니다.
...
‘표 구조·표시값을 XML로 복사’를 명시적으로 실행하면 현재 Excel 선택 영역의 셀 값·빈칸·병합 구조를, ‘셀 값·수식·참조를 XML로 복사’나 ‘AI용 간결 복사’를 실행하면 선택 영역의 셀 값·빈칸·수식·주소와 수식에 직접 적힌 같은 통합문서 A1 참조값을 로컬에서 읽어 같은 clipboard에 XML text로 기록합니다. 셀 값과 수식 내용은 로그에 남지 않고 외부 서버로 전송되지 않습니다.
