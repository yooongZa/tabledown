...
| Excel 표 범위 선택 후 메뉴 ‘마크다운 복사’ | 노트·Markdown 문서 (`Cmd+V`) | 선택한 열·표시값·줄바꿈을 유지한 표 |
...
Excel에서 제목 행을 포함한 사각형 영역을 선택하고 **‘마크다운 복사’** 를 누른 뒤 문서에 붙여넣으세요. `Cmd+C`를 먼저 누를 필요가 없습니다. 선택한 열 순서·빈 마지막 열·표시값·셀 줄바꿈을 유지하고, Markdown 원문에는 앞뒤·연속 공백과 특수문자를 보존합니다. 첫 행을 표 제목 행으로 사용하며 날짜·퍼센트·통화는 Excel에 보이는 문자열로 옮깁니다.

Markdown에서 병합 셀은 칸으로 펼쳐집니다. 색상·글꼴·열 너비·병합 모양은 Markdown 문법으로 그대로 표현할 수 없습니다. ‘빈칸을 자동 채우기’ 설정은 기존과 같이 그룹·헤더의 빈칸에만 적용하며 값 영역은 보존합니다. 함께 복사한 UTF-8 HTML 표에는 선택 영역의 병합과 줄바꿈을 담아 Excel·메모 같은 앱도 표로 붙여넣을 수 있습니다. 붙여넣는 앱이 형식을 고르며 공백의 화면 표시는 해당 앱에 따릅니다.

일반 XML과 같은 안정된 Excel 선택 읽기를 사용합니다. 최대 10,000셀·값 합계 5,000,000자, 최종 Markdown+HTML 합계 UTF-8 10MB까지 지원합니다. 읽기·변환·한도 초과는 쓰기 전에 중단하고, 준비 중 새 내용을 복사하면 그 내용을 덮어쓰지 않습니다. 성공 시 이전 Excel 전용 형식 대신 같은 선택에서 만든 Markdown과 HTML만 함께 기록합니다.

세 수동 복사 메뉴는 Excel desktop app(데스크톱 앱) 전용입니다. **Markdown은 문서의 표 모양을, 두 XML은 AI가 읽을 정보의 구조를 중심으로 정리합니다.** 기존 Excel·Google Sheets ↔ Markdown 자동 복사·붙여넣기는 그대로 사용할 수 있습니다. 아래 메뉴 안내는 2026-09-10 TestFlight 0.6.1/build 0.6.5 기준입니다.
...
기본 표 변환에는 손쉬운 사용(Accessibility)이나 입력 모니터링(Input Monitoring) 권한이 필요하지 않습니다. ‘마크다운 복사’·‘XML 변환 복사’·‘수식 포함 XML 변환 복사’는 현재 Excel 선택 영역을 직접 읽으므로 처음 사용할 때 Microsoft Excel Automation(자동화) 권한을 요청할 수 있습니다.
...
‘마크다운 복사’·‘XML 변환 복사’를 명시적으로 실행하면 현재 Excel 선택 영역의 셀 값·빈칸·병합 구조를, ‘수식 포함 XML 변환 복사’를 실행하면 선택 영역의 셀 값·빈칸·수식·주소와 수식에 직접 적힌 같은 통합문서 A1 참조값을 로컬에서 읽어 같은 clipboard에 Markdown+HTML 또는 XML text로 기록합니다. 셀 값과 수식 내용은 로그에 남지 않고 외부 서버로 전송되지 않습니다.
...
- 2026-09-10 (배포 준비): Excel에 Markdown 표를 붙일 때 줄바꿈·앞뒤/연속 공백·병합 위치를 보존하고, 큰 수식 선택의 취소 응답을 개선했다. iMac 자동 검증 350개와 실제 Excel 붙여넣기를 통과했고, 새 복사 후 취소는 5회 모두 1.7~3.9초였다. 기존 XML·자동 변환·기본 설정·크기 한도는 유지한다. 서명·스토어 업로드·공개 릴리스는 완료 후 별도로 기록한다.
