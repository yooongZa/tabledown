...
| 표 (`Cmd+C` 후 메뉴 ‘표를 XML로 변환해 복사’) | LLM 프롬프트 (`Cmd+V`) | LLM 친화 XML |
| Excel 수식 영역 선택 후 메뉴 ‘표의 수식을 포함해 XML로 복사’ 또는 `⌘⌃E` | LLM 프롬프트 (`Cmd+V`) | 셀 값·빈칸·주소·A1·R1C1 수식 XML |
...
- **표 → XML (메뉴·⌘⌃X)**: 표를 복사한 뒤 메뉴바의 **‘표를 XML로 변환해 복사’** 를 누르거나 전역 단축키 **⌘⌃X** 를 누르면 clipboard의 표(Excel 표·마크다운 표·XML)가 LLM 친화 XML로 바뀌어 다시 복사됩니다. 성공하면 메뉴바 아이콘이 1초간 체크 표시로 바뀝니다. 명시적 동작이라 어디에 붙여도 XML이 나오도록 HTML 표는 함께 제거합니다. 자동 XML→표 역변환은 두지 않습니다(워처가 일반·설정 XML을 건드릴 위험을 피하기 위함).
...
Excel에서 수식이 들어 있는 단일 사각형 영역을 선택한 뒤 메뉴바의 **‘표의 수식을 포함해 XML로 복사’** 를 누르거나 전역 단축키 **⌘⌃E** 를 누르면, 선택 영역의 모든 셀을 행·열 구조 그대로 XML text(일반 텍스트)로 복사합니다. 상수 값·빈 셀·셀 주소와 수식 셀의 현재 결과·A1·R1C1 수식이 함께 들어갑니다. 먼저 `Cmd+C`를 누를 필요는 없습니다.
...
- macOS에서는 이 메뉴를 처음 사용할 때 Excel 제어를 위한 Automation(자동화) 권한을 요청할 수 있습니다. 기존 자동 표 변환에는 이 권한이 필요하지 않습니다.
...
기본 표 변환에는 손쉬운 사용(Accessibility)이나 입력 모니터링(Input Monitoring) 권한이 필요하지 않습니다. 표의 수식을 포함해 XML로 복사하는 기능만 처음 사용할 때 Microsoft Excel Automation(자동화) 권한을 요청할 수 있습니다.
...
‘표의 수식을 포함해 XML로 복사’를 명시적으로 실행하면 현재 Excel 선택 영역의 셀 값·빈칸·수식·주소를 로컬에서 읽어 같은 clipboard에 XML text로 기록합니다. 셀 값과 수식 내용은 로그에 남지 않고 외부 서버로 전송되지 않습니다.
