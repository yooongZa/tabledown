...
- **2026-09-10 TestFlight 후속 배포 완료 (macOS 0.6.1 / build 0.6.6):** 커밋 `e07dcfa`의 iMac 검증 앱을 현재 Mac으로 가져와 기존 Mac App Store 인증서로 서명했다. 350개 자동 테스트와 실제 Excel 붙여넣기·취소·재시도를 통과한 제품 소스 16개가 최종 패키지와 일치하며, 오래된 제품 bytecode(바이트코드)와 중복 ZIP 코드는 없다. 앱·설치자 서명, App Sandbox/Excel Automation 권한, 중첩 Python의 권한 상속 및 Apple 사전 검증을 통과했다. Apple 업로드와 처리 `VALID`, 내부 그룹 `22` 연결 및 `IN_BETA_TESTING`, 한국어·영어 빌드 변경 안내 저장을 재조회했다. 기존 앱 소개와 Windows 버전은 유지했다. 업로드한 PKG의 SHA-256은 `e69e8cb155d101ccd9c5bae2299cf1d93154add68088e550cabae2c9c0ee81ed`다. 근거: `outputs/testflight/20260910-imac-fixes-0.6.6/` 및 iMac `release-20260910/testflight-0.6.6/`.


- **2026-09-10 iMac 실사용 후 macOS 복사 수정:** 명시적 Markdown HTML에 Excel용 셀 내부 줄바꿈과 조건부 공백 표현, 표 범위 표식을 추가했다. 실제 Excel에서 줄바꿈이 새 행으로 늘어나고 앞뒤·연속 공백이 축약되던 문제, 병합 셀을 붙일 때 열 배치가 틀어지던 기존 문제를 수정했다. 일반 HTML 읽기는 원래 ASCII 공백을 받고, Markdown 원문·일반 XML v2·자동 변환의 원본 HTML 유지·숫자 해석 및 기본 설정은 보존했다. 기존 Markdown+HTML 합계 UTF-8 10MB 한도를 HTML 생성 중에도 적용해 공백 표현이 커지는 단일 셀·여러 셀을 쓰기 전에 차단한다.
- **수식 읽기 취소 응답 개선:** 마스크·값·수식·참조 읽기 전후에 새 클립보드와 종료를 확인한다. 큰 선택은 기존 2,048셀 값·수식 chunk(묶음)를 나눠 호출하고 참조는 16대상·2,048셀씩 읽는다. 기존 선택·참조·문자 한도, 참조 순서, 두 연속 완전 snapshot(스냅샷) 검증을 유지하고 누적 한도를 묶음마다 초기화하지 않는다. 취소는 부분 참조 성공이나 자동 재시도로 바뀌지 않으며 수동 재시도와 마지막 쓰기 직전 세대 검증을 유지한다. 진행 중인 동기 Excel 호출은 반환 후 취소를 확인하므로 모든 환경에서 즉시 중단을 보장하지 않는다.
- **수정 검증:** iMac Python 3.13에서 격리 테스트 **273/273** 및 비-시스템-클립보드 매트릭스 **77/77** 통과(총 **350**, 시스템 클립보드 접근 0). 실제 설치 앱에서 특수문자·병합·다단 헤더 표 65셀의 공백·줄바꿈·빈열·병합 위치를 재확인했다. 4,008셀 선택 중 서로 다른 시점에 새 내용을 복사한 5회 모두 취소·새 클립보드 보존·수동 재시도가 성공했고, 취소 대기는 기존 재현 **80.804초**에서 최종 **1.700~3.885초**로 줄었다. Excel의 기존 숫자 표시 처리(예: 12.5% → 12.50%)는 유지한다. 4,008셀·수식 500개 정상 완료에서도 참조 256개와 모든 연결을 확인했다(88.722초). 테스트용 ad-hoc(임시 서명) 앱을 iMac의 Tabledown QA.app에 설치했으며 macOS 0.6.1/build 0.6.5, Windows 버전은 변경하지 않았다. 이 작업에서 TestFlight·공개 배포·커밋·푸시는 하지 않았다. 근거: iMac /Users/skynet/Tabledown-QA-20260910-menu/fix-20260910/evidence/.
...
- **Markdown 직접 복사와 왕복 보존:** 선택한 빈 마지막 열·병합이 덮는 빈 행·표시 문자열·원문 공백·특수문자·실제 줄바꿈을 보존하는 명시적 출력 옵션을 추가했다. Markdown과 UTF-8 HTML은 같은 안정된 선택에서 만들며, 과거 Excel 전용 형식 대신 두 형식과 생성 marker(표식)만 검증 후 기록한다. 기존 실행 제한·새 복사 취소·종료·실패/재시도 경로와 10,000셀·값 5,000,000자 한도를 재사용하고 두 출력 합계 10MB를 쓰기 전에 검사한다. 검토 중 발견한 병합 빈 행 누락과 특수문자 escape(이스케이프)가 역변환 값에 남는 문제를 수정했다. 자동 Excel→Markdown 출력은 유지하며 공유 Markdown→표 파서는 escape·문자참조를 한 번만 복원하고 실제 줄바꿈을 HTML까지 유지한다. Markdown의 색·글꼴·열 너비·병합 모양 표현 한계와 XML 전체 붙여넣기는 기존 한·영 도움말/README/XML 스펙/개발 지침에 반영했다.
- **2026-09-10 검증:** `.venv/bin/python -B -m unittest`로 일반 XML·수식 메뉴/reader·두 수식 serializer·Markdown 메뉴/왕복·독립 AppKit HTML importer(가져오기) 9개 모듈 **211/211**을 통과했다. 기존 비-시스템-클립보드 매트릭스 **77/77**(converter 51, i18n 11, diagnostics 9, login item 2, hotkey 4), Windows `test_windows_port` **87 pass + 27 skip**(macOS의 Windows 전용 제외), Python AST·README 실제 수식 XML 예시/참조 연결·지침 동기화·`git diff --check`도 통과했다. 특수문자 왕복과 병합 빈 행 회귀를 수정한 뒤 관련 211개 전체·converter·Windows를 재검증했다. 실제 독립 pasteboard(클립보드)에서 한글/이모지·HTML 가져오기를 확인하고 기록 실패는 대역으로 검증했으며 사용자 시스템 클립보드와 실행 중인 Excel은 조작하지 않았다. 실제 앱 메뉴 클릭·노트/Excel 붙여넣기 및 Windows 실기기 검증은 이번 작업에서 실행하지 않았다. 기존 Word 초안 2개는 해시 동일하게 보존했다.

- **2026-09-09 배포 후 기존 문서 동기화:** AGENTS·CLAUDE에 일반/수식 XML metadata(메타데이터)·클립보드 경쟁 보호의 기존 변경을 기록하고, AI용 간결 출력·UTF-8 한글 붙여넣기 보존 규칙·추가 회귀 테스트·DMG 자체 서명과 최종 검증 절차를 보완했다. README 한·영에는 GitHub Latest와 공개 다운로드 검증, App Store 상태의 확인 날짜를 명시했다. XML 스펙은 일반 XML v2의 범위를 명확히 하고 수식·간결 출력 안내로 연결했다. 새 문서·앱 코드·버전 변경은 없으며, 기존 Word 검토 초안과 주석을 보존했다. 문서 내용·로컬 링크·개발 지침 동기화·`git diff --check` 검사를 통과했다. 이번 문서 전용 작업에서는 제품 테스트를 재실행하지 않았다.


## [0.6.0] - 2026-09-09 (macOS)

macOS 배포 버전은 0.6.0, 빌드는 0.6.4다. 아래 Windows 대응 변경은 소스에 포함되며 Windows 설치 파일은 이번에 배포하지 않는다.

### Added
- **2026-09-09 AI용 간결 복사 추가 (macOS·Windows):** 기존 수식 XML 메뉴 옆에 별도 메뉴를 추가했다. 같은 안정 snapshot(스냅샷)의 선택 셀·현재 값·A1/R1C1·설명식·값 타입·계산 상태·참조 누락 사유를 보존하고, 정확히 같은 시트/주소의 참조값만 `<참조목록>`에 한 번 기록해 각 수식의 `<참조 ref="…">`로 연결한다. 선택 안의 단순한 열 제목·행 항목을 원본 주소로 연결해 `추정`으로 표시하며, 모호한 제목·다단 구조·일반 텍스트 데이터는 `미확인`으로 남을 수 있다. 기존 출력·단축키·기본 설정·선택/참조/문자/10MB 한도와 실패·새 복사 취소·종료·재시도 경로를 보존했다. 추가 Excel 읽기·재계산·네트워크 전송은 없고 작은 표는 맥락 정보 때문에 더 길어질 수 있다.
- **수식 복사 결과를 간단히 구분해 안내 (macOS·Windows).** 원본 수식과 현재 결과를 정상적으로 기록한 뒤, 일부 참조값을 가져오지 못했거나 Excel이 계산 중일 때만 짧은 완료 안내를 표시한다. 정상 macOS 복사는 기존 체크 아이콘, Windows 복사는 기존 한 번의 완료 안내를 유지한다. 계산 상태를 알 수 없다는 이유만으로 매번 안내하지 않으며, 쓰기 실패·새 복사로 취소·종료 시에는 완료 안내를 표시하지 않는다.
- **일반 표 XML에 출처·실제 병합·제목·헤더 판정 metadata(메타데이터)를 추가 (macOS).** 기존 v2 중첩 계층은 그대로 두고 `<표>` 루트에 `형식버전`·`통합문서`·`시트`·`주소`·`행수`·`열수`·`헤더행수`·`헤더기준="추정"`·`병합범위`·`제목N주소/값`을 additive attribute(추가 속성)로 기록한다. 계층은 LLM이 읽기 쉬운 structural inference(구조 추론), `병합범위`는 같은 안정 snapshot에서 얻은 실제 Excel merge topology(병합 위상)로 분리해, 같은 값이 반복된 표와 실제 병합 표를 구별한다. 2열 이상 선두 전체 폭 제목만 계층에서 제외해 metadata에 남기고 1열 세로 병합은 제목으로 오인하지 않는다. 빈·공백·중복 가로 leaf(리프) 헤더는 임의 이름 대신 원문 `n`과 보조 열 번호 `i`로 구별한다. 헤더 없는 범위를 근거 없이 자동 판정하지 않고 `헤더기준="추정"`과 “헤더 행 포함 선택” 안내로 사실성을 유지한다. `fill_blanks`가 켜지면 `빈칸채움`·`빈칸채움기준`·`빈칸채움수`·`빈칸채움셀`에 실제 생성값의 원본 A1 주소까지 기록해 보정값과 원본값을 구분한다.
- **수식 XML에 값 타입·계산 상태·정보 범위·부분 참조 사유를 추가 (macOS·Windows).** 선택 셀과 참조 셀은 `값종류=blank|number|text|boolean|error`를 기록해 실제 빈칸·빈 문자열·숫자처럼 보이는 text를 구분한다. 루트는 `계산모드`·`계산상태`·`계산결과상태`·`값기준="Excel현재원시값"`을 기록하고, 표시 문자열·숫자 서식·병합 계층을 포함하지 않는 사실도 `표시정보상태="미포함"`·`병합정보상태="미포함"`으로 자체 설명한다. Windows는 Excel의 `done/calculating/pending/unknown`, macOS는 API 한계에 맞춰 `unavailable`을 기록하며 재계산은 실행하지 않는다. 미지원·한도 초과·읽기 실패 참조에는 기존 `참조상태="일부"`와 함께 `참조포함범위수`·`참조누락이유`를 추가하고, 설명용 `값대입수식`에는 `값대입수식동등성="보장안함"`을 명시한다.
- **수식 XML에 direct reference(직접 참조) 값을 대입한 설명식을 추가 (macOS·Windows).** 원본 `수식`·`수식R1C1`·현재 결과 `값`을 그대로 유지하면서, 모든 static A1 reference(정적 A1 참조)가 안정된 snapshot(스냅샷)에서 읽힌 경우 `값대입수식` 속성으로 현재 값을 치환한 LLM용 표현을 함께 기록한다. 단일 셀은 scalar(스칼라), 범위는 행 우선 `{열,열;다음행}` 배열로 표시하고 text·boolean·오류·실제 빈칸(`BLANK()`)을 구분한다. 이는 Excel에서 재실행할 수 있거나 모든 연산에 계산 동등한 수식이라고 주장하지 않으며, 미지원·부분 참조 또는 파생식 합계 1,000,000자 초과 시 원본 export를 유지하고 속성만 생략한다. 파생식 글자 한도로 생략된 셀은 `값대입수식상태="omitted_character_limit"`을 기록해 조용한 누락을 피한다. 추가 Excel read나 recursive reference(재귀 참조)는 만들지 않는다.
- **표의 값과 수식을 함께 LLM 친화 XML로 복사하는 수동 메뉴·전역 단축키 추가 (macOS·Windows).** Excel desktop app에서 단일 사각형 영역을 선택한 뒤 ‘셀 값·수식·참조를 XML로 복사’를 실행하거나 macOS **⌘⌃E** / Windows **Ctrl+Alt+E** 를 누르면, 모든 셀의 값·빈칸·주소와 수식 셀의 현재 결과·A1/Formula2·R1C1 표현을 행·열 구조 그대로 XML text에 기록한다. Tabledown은 수식을 실행하지 않고 Excel의 현재 값을 읽으며, 자동 clipboard watcher의 기존 Excel↔Markdown 동작은 변경하지 않는다. macOS는 Excel Apple Events Automation, Windows는 실행 중인 Excel COM을 사용하고, 결과 clipboard는 명시적 text-only export로 교체한다.
...
### Changed
- **공유 수식 참조를 고유 대상 기준으로 한 번만 읽도록 변경 (macOS·Windows).** 여러 수식 셀이 같은 `(sheet,address,row_count,column_count)` static A1 target(정적 A1 대상)을 참조하면 Excel read는 한 번만 수행하고 같은 immutable reference(불변 참조)를 모든 수식 셀에 연결한다. 256범위·10,000셀 한도도 고유 대상에만 적용해, 동일 참조 수백 개가 불필요하게 한도를 소진하거나 Apple Event/COM round-trip(왕복)을 반복하지 않는다. owner 수집은 set 기반이라 10,000 수식 셀에서도 O(n²)로 느려지지 않고, XML에는 각 수식에 참조가 나타난 순서를 유지한다. 겹치지만 주소가 다른 범위는 합치지 않으며, 공유 대상 읽기 실패는 모든 owner cell(소유 수식 셀)에 동일한 사유로 표시한다.
- **수식 XML 10MB 상한도 생성 중 적용.** 공유 참조가 많은 선택에서 동일 `<참조범위>`가 owner마다 출력되더라도 완성된 ElementTree(요소 트리)를 먼저 만들지 않고 bounded serializer(제한 직렬화기)가 각 줄을 추가하기 전에 UTF-8 byte를 검사한다. 한도 초과 시 설명용 `값대입수식`만 제외해 한 번 재시도하고 루트에 `값대입수식상태="omitted_xml_size_limit"`을 기록한다. 그래도 크면 `값종류`·계산 상태·참조 누락 사유를 조용히 삭제하지 않고 clipboard writer 전에 실패한다.
- **일반 XML도 UTF-8 10MB 상한을 생성 중 적용.** 헤더가 데이터 행마다 반복되는 형식에서 완성 후 크기만 검사해 거대한 문자열을 먼저 만들던 위험을 없애고, bounded serializer(제한 직렬화기)가 각 줄을 추가할 때 byte(바이트)를 계산해 즉시 중단한다. 한도 초과는 clipboard writer 전에 전용 안내로 끝나며 기존 clipboard를 보존하고 성공 표시 없이 실패한다.
- **macOS XML 내보내기의 진행 상태와 single-flight(단일 실행) 제어를 추가.** 메뉴 callback(콜백)은 먼저 ‘복사 중…’ 상태를 적용하고 두 XML 항목을 잠시 비활성화한 뒤, 다음 main run loop(메인 실행 루프) turn에 Excel 읽기를 시작한다. 두 명령은 하나의 gate를 공유해 중복 요청을 안내하고, 종료 전에 예약된 작업은 clipboard와 UI를 쓰지 않는다. `NSAppleScript`는 macOS에서 main-thread-only(메인 스레드 전용)이므로 Excel 읽기 자체를 background thread로 옮기지 않았으며, 큰 범위를 읽는 동안 메뉴가 잠시 기다릴 수 있다는 제한을 도움말에 명시했다. 자동 변환 불변식과 기존 단축키는 유지한다.
- **두 XML 메뉴의 입력·출력 차이를 이름과 도움말에 명시 (macOS·Windows).** 일반 명령은 ‘표 구조·표시값을 XML로 복사’, 수식 명령은 ‘셀 값·수식·참조를 XML로 복사’로 바꿨다. 전자는 표시값·빈칸·병합 계층을 보존하지만 수식 text를 포함하지 않고, 후자는 셀 주소·값·수식·직접 참조를 보존하지만 병합 계층·표시 서식을 포함하지 않는 차이를 앱 도움말과 README에 동기화했다.

### Fixed
- **Markdown 표를 Apple 메모에 붙일 때 한글이 깨지던 문제 수정 (macOS·공유 변환기).** 새로 만드는 HTML에 `<meta charset="utf-8">`을 선언해 메모의 문자 인코딩 추측을 방지한다. Markdown 원문·셀 값·특수문자 escape(이스케이프)와 기존 Excel·웹 HTML 보존 동작은 유지한다. Windows도 같은 변환기를 사용한다.
- **macOS 일반 XML: 선택 이동 중 읽기 실패 복구.** 값·병합을 읽다가 실패하더라도 처음 기록한 통합문서·시트·주소와 현재 선택을 확인한다. 선택 변경이 확인되면 기존 최대 3회·연속 두 스냅샷 일치 절차로 재시도한다. 같은 선택의 읽기 실패·확인 불가·권한 거부는 기존 오류를 유지하고, 정상 조회 수·표시값·병합·출력 형식·상한·클립보드 보존 동작을 바꾸지 않는다.
- **macOS Excel 실행 감지 보강.** iMac 실제 단축키 검사에서 Excel 프로세스가 유지되는 동안 bundle ID 조회가 비어 미실행 안내가 나온 사례 2회를 확인했다. 첫 조회가 비면 `NSWorkspace.runningApplications`에서 정확한 Excel 식별자와 미종료 상태를 추가 확인한다. 정상 첫 조회·실제 미실행 차단을 유지하며 Excel 자동 실행·추가 AppleScript·대기·전역 캐시는 넣지 않는다. 최초 조회가 비었던 macOS 내부 원인은 확인하지 못했다.
- **빈 상위 헤더 아래의 중간 제목 보존.** 일반 XML에서 빈 병합 헤더 아래의 ‘1분기·2분기’ 같은 중간 그룹 이름이 사라지던 문제를 수정했다. 빈 레벨 아래를 같은 열 묶음으로 탐색하며, 기존 leaf(리프) 헤더 원문·중복 열 번호·중첩 형식·실제 병합 metadata(메타데이터)는 유지한다.
- **수식 참조 오인과 누락 판정 수정.** 한글을 포함한 정의 이름 내부의 `A1`을 독립된 셀 주소로 읽지 않는다. `A1:INDEX(...)`처럼 계산된 범위 끝점은 추측하지 않고 일부 참조로 표시하며 설명용 값대입수식을 생략한다. 원본 수식·현재 결과와 일반 `INDEX` 조회·정적 A1 참조는 보존한다.
- **Windows 참조 좌표 읽기 실패 복구.** 참조 범위의 행·열 좌표를 읽지 못해도 선택한 원본 수식과 결과는 유지하고, 해당 참조만 `read_failed`로 기록해 일부 복사로 처리한다.
- **자동 변환 중 도착한 더 새 clipboard를 이전 결과로 덮거나 섞던 경쟁 상태 수정 (macOS·Windows).** watcher가 source change count/sequence(원본 변경 번호)를 읽기 직전에 잡아 preserving writer(형식 보존 기록기)에 전달한다. 변환 중 새 복사가 들어오면 이전 변환을 쓰지 않고 해당 generation을 소비하지 않아 다음 tick이 최신 복사본을 처리한다. Windows는 열린 clipboard 안에서 비교해 writer와 배타적으로 처리하고, macOS는 `clearContents` 직전까지 한 번 더 확인해 OS API가 허용하는 범위에서 TOCTOU window(검사-사용 창)를 줄였다.
- **명시적 XML export와 자동 clipboard watcher의 TOCTOU race condition(검사-사용 경쟁 상태) 수정 (macOS·Windows).** export 시작 신호를 clipboard operation lock(클립보드 작업 잠금) 안에서 게시하고 watcher가 잠금 획득 뒤에도 다시 확인한다. export 중 들어온 새 clipboard generation(세대)은 watcher가 미리 소비하지 않으므로, export가 새 복사본 보호를 위해 취소된 뒤 다음 tick에서 정상 자동 변환된다. 새 clipboard 취소·10MB 출력 초과·검증된 clipboard 쓰기 실패는 서로 다른 내용 비노출 안내로 분리하고, clear 이후 실패를 자동 재시도해 더 새 내용을 덮지 않는다.
- **느린 Excel XML 내보내기가 사용자가 새로 복사한 clipboard를 덮어쓰거나 실패를 성공으로 표시할 수 있던 경로 수정 (macOS·Windows).** 작업 시작 시 clipboard change count/sequence(변경 번호)를 snapshot하고 text-only writer가 비우기 직전에 같은지 확인해, 중간에 새 내용이 복사되면 writer를 호출하지 않고 취소 안내를 표시한다. 성공은 plain text XML과 생성 marker(마커)를 정확히 read-back(재확인)한 뒤에만 보고한다. OS clipboard는 clear/empty 뒤 transaction(트랜잭션)이나 atomic rollback(원자적 되돌리기)을 제공하지 않으므로, 부분 쓰기 실패는 재시도·복구로 새 내용을 덮지 않고 내용 없는 오류로 처리한다. 자동 Excel↔Markdown watcher의 기존 format 보존 동작은 변경하지 않았다.
- **Windows 수식 XML 명령을 실행 중 다시 눌러도 아무 반응이 없던 문제 수정.** 기존 single-flight lock(단일 실행 잠금)은 유지하면서 진행 중임을 명확히 안내한다. 명시적 export가 Excel을 읽는 동안 watcher가 새 clipboard generation(세대)을 먼저 소비하지 않게 해, 새 복사는 export 취소 후 다음 watcher tick(감시 주기)에 정상 처리된다.
- **macOS 단일 수식 셀의 현재 결과와 직접 참조값이 빠지거나 다른 셀 값으로 읽히던 문제 수정.** Excel AppleScript가 명시적 1-cell Range에 `value2/has formula of every cell`을 호출하면 실제 값·`true` 대신 빈 값·`false` 또는 인접 셀 값을 반환하는 동작을 실제 Excel `수식_스냅샷!F10`에서 재현했다. 1셀일 때만 Range scalar(스칼라) `value2`·`has formula`를 읽어 1-item list로 만들고, 2셀 이상은 기존 batch(일괄) 경로를 보존했다. 선택값·직접 참조값 모두 `value`가 아닌 `value2`를 사용해 날짜·통화를 지역화된 표시 text가 아니라 raw numeric value(원시 숫자값)로 분류한다. serializer도 수식 셀 결과가 `None`이면 조용히 불완전 XML을 만들지 않고 writer 호출 전에 fail-closed(실패 시 차단)한다.
- **수식 XML 숫자의 scientific notation(지수 표기)을 일반 십진수로 수정 (macOS·Windows).** macOS AppleScript `as text`는 `315000`을 `3.15E+5`로 축약했고, Windows `str(float)`는 극대·극소값을 지수로 쓰거나 정수형 값에 불필요한 `.0`을 남길 수 있었다. 이제 Excel native numeric type(네이티브 숫자 타입)만 `Decimal` 기반으로 `315000`·`0.00000012`처럼 펼치고 불필요한 `.0`을 제거한다. literal `3.15E+5` text는 타입을 구분해 원문 그대로 보존하며, 선택값·수식 결과·같은/다른 시트 직접 참조값에 동일하게 적용한다.
- **A1 reference parser(참조 파서)의 공백 포함 3-D 범위·지수 상수·Excel 오류 리터럴 오인 수정.** `Sheet1:Sheet3!A1 : B2`의 뒤쪽 `B2`를 현재 시트 직접 참조로 잘못 읽던 경로를 전체 3-D 표현 masking(마스킹)으로 차단해 불필요한 셀 읽기를 막았다. `1E+5`·`1.2E-7` 같은 유효 numeric literal(숫자 리터럴)과 `#N/A`·`#DIV/0!`·`#NAME?` 같은 오류 리터럴을 defined name(정의된 이름)으로 오인하지 않아, 함께 있는 static A1 reference의 완전한 값대입 표현을 유지한다.
- **일반 표→XML을 clipboard 사전 복사 방식에서 Excel 직접 선택 방식으로 수정 (macOS).** 이제 Excel에서 단일 사각형 표 범위를 선택하고 ‘표 구조·표시값을 XML로 복사’ 또는 **⌘⌃X** 를 실행하면 `Cmd+C` 없이 Excel range의 `string value`로 데이터 셀 text의 유의미한 공백과 숫자·날짜·퍼센트·통화·사용자 지정 표시값, 빈칸·오류값·병합 구조를 직접 읽는다. clipboard 표 fallback을 제거해 오래된 복사본을 변환하던 UX를 차단했다. 선택·값·병합 영역이 같은 immutable snapshot(불변 스냅샷) 두 개가 연속될 때만 내보내며(최대 3회), Excel 미실행·Automation 거부·다중 영역·**10,000셀 초과**·선택 경계를 걸친 병합·읽는 중 변경은 fail-closed(실패 시 차단)로 writer 호출 전 기존 clipboard를 보존한다. 혼합 범위에서 거짓을 돌려주는 Excel의 merge scalar(병합 스칼라) 대신 모든 셀의 merge area 주소를 batch(일괄)로 읽어, 정확한 병합 구조와 수식 XML 명령과 같은 10,000셀 한도를 유지한다. HTML 구조 추론에는 opaque token(불투명 토큰)을 써 데이터 셀의 앞뒤·연속 공백과 literal `<br>`를 보존하고, XML 1.0 금지문자는 clipboard 쓰기 전에 차단한다. 기존 자동 Excel/Sheets↔Markdown watcher는 변경하지 않았다.
...
### Verification
- **2026-09-09 정식 macOS 설치 파일 검증:** TestFlight와 같은 소스 91개 및 번들 Python 소스 16개 일치를 확인했다. 앱과 DMG의 Developer ID 서명·Apple 공증 Accepted·stapled ticket(부착된 공증 티켓)·Gatekeeper 검증을 통과했다. ZIP을 동기화 폴더 밖에 새로 풀고 DMG를 읽기 전용으로 마운트해 내부 앱 서명·소스·버전 0.6.0/build 0.6.4도 확인했다. 배포 대상은 Apple Silicon이다. 기존 빌드 스크립트가 DMG 자체 서명을 생략해 이번 DMG 산출물에 서명을 추가한 후 재공증했다. 원본 빌드 스크립트의 자동화 보완은 후속 항목이며 이번 최종 DMG 검증은 통과했다.
- **2026-09-09 TestFlight 배포 후보 검증:** 별도 소스 복사본과 Python 3.13.2 환경에서 macOS/shared 단위 146/146 및 converter·i18n·diagnostics·login item·hotkey 77/77(총 223개)을 통과했다. 원본 소스 91개 해시, 번들 소스 16개 일치·오래된 bytecode 없음·tests 패키지 제외·버전/권한/프로파일·앱 및 설치자 서명을 확인했다. 같은 원본 패키지를 동기화 폴더 밖에서 다시 풀어 속성 제거 없이 deep strict 서명을 검증했고 Apple validate-app도 오류 없이 통과했다. Documents 안에서 해제할 때 추가된 FinderInfo 문제는 별도 증거로 보존했다. Excel 접근 허용 문구는 선택한 표와 계산을 복사하는 사용 목적 중심으로 보정했다. 기능·기본 설정·기존 설치 및 Windows 버전은 변경하지 않았다.
- **2026-09-09 AI용 간결 복사 검증:** 공용 변환·macOS 동작/한글 인코딩 단위 테스트 146/146, converter 51/51, i18n 11/11, Windows 포트 114건 중 87 pass + 27 skip(macOS에서 플랫폼 전용 제외)을 통과했다. 기존 수식 XML은 정상 252개·크기 경계 80개 비교에서 출력 또는 오류가 동일했고, 새 XML에서 기존 선택 셀의 모든 속성과 수식별 참조를 재구성했다. 중복 실행·읽기/변환/쓰기 실패·새 복사 취소·종료·재시도·크기 제한·추정 실패 경로도 검증했다. 실제 로컬 Excel의 가상 통합문서를 설치된 앱의 읽기/변환 소스로 검증해 16셀·3개 수식의 한글/이모지·결과 300/400/150·맥락 연결과, 다른 시트 동일 범위를 공유하는 8개 수식의 결과 850·공통 범위 1개·참조 셀 3개·연결 8개를 확인했다. 두 출력의 핵심 정보는 같고 후자는 4,562→3,693 UTF-8 byte(약 19.0% 감소)였다. 실제 메뉴 클릭은 메뉴바 앱에 접근하지 못한 CUA 도구 제약으로 미검증이며 Windows 실기기도 미검증이다. 별도 가상 6사례·고정 24문항의 같은 모델/설정 단회 AI 비교에서는 기존/간결 출력 모두 24/24로, 일반적인 정확도 향상을 입증한 결과로 해석하지 않는다. 100개 수식이 같은 100셀을 참조하는 가상 사례는 706,103→58,540 byte(91.7% 감소)였다.
- **2026-09-09 한글 붙여넣기 수정 검증:** 실제 clipboard writer(클립보드 쓰기 함수)와 독립 pasteboard(클립보드)의 두 HTML 형식을 AppKit으로 읽는 회귀 테스트를 추가했다. 수정 전 두 형식 모두 한글 깨짐을 재현했고 수정 후 1개 테스트가 통과했다. converter 51/51, Windows 포트 107개 중 80 pass·27 skip(macOS 실행)이 통과했다. 재설치한 macOS 앱에서 TextEdit 원문 복사 → Apple 메모·Excel 붙여넣기로 한글·이모지·특수문자 4행 2열의 8개 셀을 확인했고, Excel → Markdown 재복사도 확인했다. Windows 실기기는 이번 변경에서 검증하지 않았다.
- **2026-09-07 Microsoft 365 활성화 후 추가 실기기 검증:** iMac의 기존 Excel 프로세스를 정상 재시작한 뒤 셀 편집·붙여넣기를 확인했다. 최종 0.6.0 QA 앱 재빌드·설치와 앱/ZIP 서명 검증, 단위 **154/154**·비클립보드 매트릭스 **77/77**을 통과했다. 실제 복사·붙여넣기 7항목과 Excel 25셀·Markdown 6셀 전수 값/타입 비교, 일반 XML 714자·31줄 및 수식 XML 3817자·99줄의 Excel/TextEdit 보존을 확인했다. 4,008셀·1,000수식 읽기 중 J2를 2→3으로 바꿔 전체 수식·값·참조의 일관성과 2 복구를 확인했다. 선택 이동 중 최초 일반 읽기 오류는 새 빌드에서 정확한 선택 변경 안내·클립보드 보존으로 개선됐고 재시도도 성공했다. 두 경합은 실제 AppleScript 실행 스택·변경 ACK·완료 시각으로 확인했다. Excel 미실행·재실행·Tabledown 재실행 7항목, 일시정지 후 재실행 시 자동 변환 복구, 최종 병합/서식 출력 일치·표시 넘침 거부도 통과했다. 활성화 전 미완료 기록과 검증 도구의 실패/경합 미관측 이력은 별도 보존했다. 남은 설치 항목은 기존 0.5.0 앱 제거의 관리자 인증이며, 부분 병합 거부 재현·Windows 실기기는 이번 범위에서 미확인이다. 근거: iMac QA evidence의 `FINAL-RESULT.json`·`REPORT.html`.
- **2026-09-07 iMac SSH 실기기 검증:** macOS 26.6.2 arm64·Excel 16.112.3에서 최신 0.6.0 ad-hoc QA 앱을 빌드·설치했다. 단위 148/148, 비클립보드 matrix 77/77, 앱·ZIP 재추출본 deep strict 서명, bundled source/bytecode 16/16 일치 검증을 통과했다. 일반/수식 XML 최신 출력 22개(초기 실행 감지 실패 2개와 성공한 재시도 이력 별도 보존), 감지 보강 후 시트를 바꾸는 단축키 20회, 표시 넘침·수식 없음·일반/수식 10,001셀·비연속 선택의 클립보드 보존 5경로를 실제 메뉴·키보드로 확인했다. 4,008셀은 표 약 12초·수식 약 21초였고, 표 읽기 시작 약 2.16초 뒤 TextEdit에서 새 내용을 실제 복사하자 새 내용을 유지하고 약 12.65초에 취소 안내했다. 자동 복사의 Markdown+HTML 공존, Markdown에서 HTML 보강, XML→TextEdit 714자 일치도 확인했다. **남은 검증/설치:** 기존 `/Applications/Tabledown.app` 0.5.0 제거에는 관리자 인증이 필요해 새 앱은 `/Applications/Tabledown QA.app`에서 실행 중이다. Excel은 Microsoft 365 활성화/읽기 전용 안내로 셀 쓰기를 받지 않아 Excel 대상 붙여넣기는 완료로 처리하지 않았다. 부분 병합 선택은 Excel이 전체 병합으로 확장해 실제 거부를 재현하지 못했고, 값 변경 중 스냅샷·Windows 실기기는 미검증이다. 근거는 iMac QA evidence의 `FINAL-RESULT.json`·`REPORT.html`에 기록했다.
- **2026-09-07 XML·수식 복사 완성도 보강 검증:** macOS 관련 단위 테스트 **143/143**, 시스템 clipboard를 사용하지 않는 converter·i18n·diagnostics·login item·hotkey 매트릭스 **77/77**(converter **51/51** 포함), Windows 포트 **106건 중 79 pass + 27 skip**을 통과했다. 빈 상위/중간 헤더의 그룹 이름·원문 값·이웃 그룹 경계, 한글 정의 이름과 정상 시트명, 함수·괄호·깨진 참조로 정해지는 범위 끝점, 오류값과 나눗셈, 부분 복사/계산 중 안내의 한국어·영어 문구, Windows 참조 좌표 오류 복구, 새 복사 취소·쓰기 실패·종료 시 완료 안내 억제·실패 후 재시도를 검증했다. 실제 동작 메서드에 Excel/clipboard 대역을 연결한 경로로 확인했으며, 현재 실행 중인 Excel·실제 clipboard와 Windows 기기는 조작하지 않았다. 변경 Python AST(구문) 검사와 `git diff --check`도 통과했다. 기존 미커밋 변경·버전·단축키·설정 기본값·출력 한도는 보존했다.
- **2026-08-08 앱 사양서 검증:** `쉽게 말하면` 주석 mapping(대응) 55/55와 요구사항 바로 아래 배치를 기계적으로 확인했다. Microsoft Word 실제 렌더 45쪽을 전수 확인해 한글 font(글꼴)·설명 상자·표·페이지 분할·6개 comment anchor(주석 앵커)의 잘림/겹침이 없음을 확인했다. DOCX 14개 표는 `tblW`·`tblInd`·`tblGrid`·`tcW`가 모두 일치했고, comment 6/6, accessibility audit(접근성 검사) high/medium/low 0/0/0, heading hierarchy(제목 계층), ZIP 무결성을 통과했다. 앱 회귀는 사양서 최초 작성 때 converter 51/51, macOS unit 128/128, Windows port 100건 중 73 pass + 27 skip(macOS host의 Windows tray·WinRT 전용)을 통과했으며, 이번 후속 작업은 문서만 수정해 앱 테스트를 다시 실행하지 않았다. 실제 Windows Excel/COM 및 최신 실제 macOS Excel 메뉴 수동 검증은 문서 작성 범위에서 실행하지 않았다.
- **2026-07-30 사용자 관점 XML 내보내기 개선 검증:** 공용 formula serializer(수식 직렬화기)·macOS Excel table/formula reader·메뉴 orchestration(오케스트레이션)·text-only clipboard writer 집중 테스트 128/128, converter 불변식 51/51, 기본 test matrix 82/82, Windows 포트 100건 중 73 pass + 27 skip(macOS에서 Windows tray·WinRT 전용 제외), 변경 Python `compileall`, `git diff --check`를 통과했다. 정상 쓰기·빈칸 보정 provenance(출처)·병합 source address(원본 주소) deduplication(중복 제거)·메뉴 예약·설정 snapshot·중복 실행·selection 재시도·새 clipboard 취소·write/read-back 실패·종료 전 취소·두 종류의 파생 수식 생략 상태를 검증했다. 4,990,000자 단일 참조를 10,000개 수식이 공유하는 적대적 모델은 0.21초에 10MB 초과를 bounded fail(제한 실패)했고 maximum resident set size(최대 상주 메모리)는 85,360,640byte였다. `NSAppleScript`의 main-thread-only(메인 스레드 전용) 제약 때문에 macOS Excel read를 위험한 background thread로 옮기지 않았다. 실제 Windows Excel/COM과 실제 macOS 메뉴의 대용량 선택 수동 검증은 실행하지 않았다.
- **2026-07-21 숫자 원문·값대입수식 최종 검증:** 공용 formula serializer(수식 직렬화기)·macOS formula reader(수식 판독기)·기존 일반 Excel→XML 회귀 86/86, 기본 test matrix 82/82, Windows 포트 89건 중 68 pass + 21 skip(macOS에서 Windows tray·WinRT 제외), 변경 Python compile(컴파일), `git diff --check`를 통과했다. 실제 macOS Excel에서 `수식_스냅샷!F10`의 `=SUM(F4:F9)`을 연속 두 snapshot으로 다시 읽어 현재 결과 `862300`, 참조값 `75000·136800·315000·72000·263500·0`, `값대입수식="=SUM({75000;136800;315000;72000;263500;0})"`가 지수 표기 없이 생성되고 clipboard text-only roundtrip(왕복)이 일치함을 확인했다. 실제 Windows Excel/COM 기기 검증은 실행하지 않았다.
...
### Distribution
- **2026-09-09 macOS 0.6.0 공개 배포 완료:** [GitHub v0.6.0](https://github.com/yooongZa/tabledown/releases/tag/v0.6.0)을 Latest로 공개했다. 인증 없는 공개 API와 실제 DMG·ZIP 다운로드에서 크기·SHA-256이 검증 원본과 일치했고, README 고정 다운로드 링크가 새 DMG로 연결됨을 확인했다. 기존 v0.5.0 자산은 유지됐다. App Store build 0.6.4는 최종 재조회에서도 심사 대기(`WAITING_FOR_REVIEW`)·승인 후 자동 공개(`AFTER_APPROVAL`)이며, 한국어 설명·변경 안내가 확정 원문과 일치했다. 검증 근거는 `outputs/releases/20260909-0.6.0-160512/`에 보존했다.
- **2026-09-09 macOS 0.6.0 정식 배포 준비 및 App Store 심사 제출:** 검증된 build 0.6.4를 정식 버전에 연결하고, 확정된 한국어 원문과 영어 설명을 저장·재조회했다. App Store Connect에서 `WAITING_FOR_REVIEW`와 승인 후 자동 공개(`AFTER_APPROVAL`)를 확인했다. GitHub `v0.6.0`용으로 서명·공증된 `Tabledown.dmg`(SHA-256 `7960ff780c109e65dba556e5d1eaaf3d9ed24fcdf085aa28538152036c05bb6c`)와 `Tabledown-notarized.zip`(`c4b05952f120b369e204de092691e2a28b3451de90f6638fe4ea2ffa61b2bc19`)를 준비했다. App Store 사용자 공개는 Apple 심사 승인 이후 진행된다.
- **2026-09-09 macOS 0.6.0 / TestFlight build 0.6.4 업로드:** Apple이 15:29:47 KST에 15,150,033-byte 패키지를 오류 없이 수신했다(delivery `a97de734-8627-4f4a-b278-43bace7dc10e`, SHA-256 `09b0a9819ba2ef64cd4fe121ebec4833c5d4fb438b2cc6f639816c1e06326f15`). 한국어·영어 베타 앱 설명을 값/계산을 따로 모으는 수고와 표 재정리 시간을 줄이는 편의 중심으로 저장·재조회했다. 15:35 KST에 처리 `VALID`·기존 내부 그룹 `22` 연결·`IN_BETA_TESTING`을 확인했다. 한국어·영어 변경 안내도 새 빌드에 저장한 뒤 원문과 정확히 일치함을 재조회했다. 한국어·영어 베타 설명과 변경 안내는 앱 전체의 편의를 소개한다: Excel/Sheets 표를 노트·문서로 옮기기, 마크다운 표를 Excel에서 이어서 편집하기, 복잡한 표의 제목·묶인 항목·표시값을 설명 자료로 준비하기, 평소 복사·붙여넣기를 유지하기. 수식 복사는 보조 기능으로 포함하고 한글 메모 개선은 이번 버전 소식으로 구분했다. 수정한 4개 설명 필드는 저장 뒤 원문 일치를 재확인했으며 빌드는 변경하지 않았다. 한국어 최종 설명과 변경 안내는 사용자가 확정한 “당신의 AI를 더 스마트하게 사용하세요.” 원문으로 교체했다. 단어·표기·표현을 수정하거나 추가 안내를 붙이지 않았으며, TestFlight 일반 텍스트에 맞춰 목록과 줄바꿈만 적용했다. 한국어 2개 필드의 저장값 일치를 재확인했고 영어와 build 0.6.4는 유지했다. 공개 App Store 심사 제출·GitHub Release·기존 설치 교체는 수행하지 않았다. 근거: `outputs/testflight/20260909-0.6.4-151436/`.
- **2026-09-09 AI용 간결 복사 로컬 설치 (macOS 0.6.0):** 기존 설치본과 설정을 백업하고 `app.py`·`i18n.py`·`converter/formula_export.py` 3개 소스만 반영한 후보로 `/Applications/Tabledown.app`을 교체·실행했다. Python 3.13 실행 환경·의존성·버전·기존 설정을 보존했으며, 파일 비교에서 3개 소스와 서명 파일만 변경됨을 확인했다. `codesign --verify --deep --strict`와 설치 소스 일치·실행 프로세스 확인을 통과했다. 전체 재빌드·공개 배포·원격 iMac 설치는 수행하지 않았다.
- **2026-09-09 로컬 macOS 재설치:** 기존 iMac 검증 ZIP의 Python 3.13 실행 환경·의존성·macOS 버전 0.6.0을 보존하고 `md_to_tsv.py`만 교체한 뒤 앱 서명을 갱신했다. ZIP 재추출과 설치 후 `codesign --verify --deep --strict`를 통과했으며 기존 앱·설정을 백업하고 `/Applications/Tabledown.app`을 교체·실행했다. 전체 소스 재빌드나 공개 배포는 수행하지 않았다.
