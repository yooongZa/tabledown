...
- **메뉴 구조 정리 (UX P2)**: ‘설정 ▸’ 을 후원 위로(첫인상은 유틸리티, 후원은 그 아래), ‘구매 복원’ 을 설정 ▸ 에서 후원 서브메뉴 하단으로 이동(결제 관련 항목 한곳에). 후원 감사 알럿에 전용 타이틀. 변환 토글(`enabled`)은 **의도적으로 비영속**(매 실행 켜짐 — "꺼둔 걸 잊고 고장 오인" 방지) — 정책을 CLAUDE.md "설정 영속성 정책" 에 명문화.
...
- **UX 정리 1차 — 무반응 제거 (P0, `outputs/tabledown-uxui-plan.md`):**
  - **결제 흐름 피드백**: 구독 성공("Pro 활성화 완료" — 방금 시작한 구독에만, 자동갱신·복원은 침묵)·구매 실패(사용자 취소는 침묵)·**구매 복원 결과**(복원됨/내역 없음/실패) 알럿. 기존엔 전부 무반응이었음.
  - **가격 동적 표기**: 기부 메뉴에 스토어 로컬라이즈 가격 병기("커피 한 모금 ☕ (₩3,000)" 식), 구독 시트의 하드코딩 "$4.99" 는 메타데이터 로딩 전 fallback 으로 강등. 시작 직후 네트워크 레이스 대비 상품 fetch 재시도(2회).
  ...
  - **Pro 잠금 표시**: 비구독 상태에서 ‘표를 XML로 복사 🔒’ — 클릭 전에 Pro 필요함이 보이고, 구독/복원 즉시 해제.
- **UX 정리 2차 — 첫인상 (P1):**
