...
### 2. 셀개수 검사(strict)는 html `<table>` 이 "없는" 순수 텍스트에만
- 셀개수 일치 검사의 **유일한 목적은 false positive(거짓양성) 차단** — 셸 출력·ASCII 박스처럼
  우연히 `| ... |` 모양이고 둘째 줄이 `-` 로 시작하는 일반 텍스트를 표로 오인하는 것 방지.
- html `<table>` 이 있으면 이 목적이 불필요(이미 진짜 표 증거)하므로 검사를 끈다(불변식 1).
...
- html 제거가 정당한 **유일한** 경우: **순수 Excel/Sheets 표 → Obsidian.** (html 을 남기면
  에디터가 리치 표로 붙여 마크다운이 안 되므로 — 커밋 `7c677b8`.)
