...
- **Table → XML (menu or ⌘⌃C)**: Copy a table, then click **“Copy table as XML”** in the menu bar or press the global shortcut **⌘⌃C** to turn the clipboard table (Excel table, Markdown table, or XML) into LLM-friendly XML. On success the menu bar icon briefly shows a checkmark. This is a deliberate action, so the HTML table is dropped and XML pastes everywhere. There is no automatic XML→table direction (to avoid the watcher touching ordinary config/document XML).
